		...
		var methods ={
			...
			sendQuery : function(){
				...
				if(_this)
				{
					...
					if(action.indexOf("?")>=0)
					{
						getUrl = action+"&"+nameMainVar+"=Y&ajax_basket=Y&action=calculate&entity="+entity+"&s_id="+basketItemId+"&bxajaxid="+ajaxID;
					}else{
						getUrl = action+"?"+nameMainVar+"=Y&ajax_basket=Y&action=calculate&entity="+entity+"&s_id="+basketItemId+"&bxajaxid="+ajaxID;
					}
					...
					BX.ajax.insertToNode(getUrl+"&props="+strProps+"&offerID="+offerID, 'comp_'+ajaxID);
				}else{
