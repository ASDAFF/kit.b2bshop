		...
		<div class="col-sm-19 sm-padding-right-no">
			<div class="login_page">
				...
				<form name="form_auth" method="post" target="_top"
				      action="<?= SITE_DIR ?>auth/<? //=$arResult["AUTH_URL"]?>" class="bx_auth_form">
					<input type="hidden" name="AUTH_FORM" value="Y"/>
					<input type="hidden" name="TYPE" value="AUTH"/>
					<? if(strlen($arParams["BACKURL"]) > 0 || strlen($arResult["BACKURL"]) > 0): ?>
						<input type="hidden" name="backurl"
						       value="<?= ($arParams["BACKURL"] ? $arParams["BACKURL"] : $arResult["BACKURL"]) ?>"/>
					<? endif ?>
						...
						<div class="wrap_block">
							...
							<div class="row">
								...
								<div class="col-sm-17 col-md-10 col-lg-11 sm-padding-left-no">
									<input class="login_input" type="text" name="USER_LOGIN" maxlength="255"
									...
									<div class="wrap_remember_forgot">
										<? if($arResult["STORE_PASSWORD"] == "Y"): ?>
											<div class="rememberme">
												<input type="checkbox" id="PAGE_USER_REMEMBER" name="USER_REMEMBER"
												       value="Y" checked/>
												<label class="label-active"
												       for="PAGE_USER_REMEMBER"><?= GetMessage("AUTH_REMEMBER_ME") ?></label>
											</div>
										<? endif ?>
										<? if($arParams["NOT_SHOW_LINKS"] != "Y"): ?>
											<a class="forgot_password" rel="nofollow"
											   href="<?= $arParams["AUTH_FORGOT_PASSWORD_URL"] ? $arParams["AUTH_FORGOT_PASSWORD_URL"] : $arResult["AUTH_FORGOT_PASSWORD_URL"] ?>"
						...
						<div class="wrap_block">
							<div class="row">
								<div class="col-sm-7 col-md-6 col-lg-5 sm-padding-right-no">
									<div class="wrap_btn">
										<input type="submit" name="Login" class="login_btn"
