...
{
    if(isset($_COOKIE["sotbit_filter"][$Item['LINK']]) && !empty($_COOKIE["sotbit_filter"][$Item['LINK']]))
    {
        $arResult[$key]['OPENS']=$_COOKIE["sotbit_filter"][$Item['LINK']];
    }
}
...
if(Loader::includeModule('sotbit.seometa'))
{
	...
	if($uri)
	{
		$seometaWorking = \Sotbit\Seometa\SeometaUrlTable::getList(
				array(
						'filter' => array(
								'ACTIVE' => 'Y',
								'REAL_URL' => $uri
