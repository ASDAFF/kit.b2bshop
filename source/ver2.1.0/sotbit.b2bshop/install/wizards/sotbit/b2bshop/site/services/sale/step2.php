...
if(COption::GetOptionString("sotbit.b2bshop", "wizard_installed", "N", WIZARD_SITE_ID) != "Y")
{
	$locationGroupID = 0;
	...
	if(!$dbDelivery->Fetch())
	{
		...
		{
			$logo = $_SERVER["DOCUMENT_ROOT"].WIZARD_SERVICE_RELATIVE_PATH ."/images/kurier.png";
			$arPicture = CFile::MakeFileArray($logo);
			$arFields['LOGOTIP'] = $arPicture;
		}
		...
		if($module == 'sotbit.b2bshop')
		{
			$logo = $_SERVER["DOCUMENT_ROOT"].WIZARD_SERVICE_RELATIVE_PATH ."/images/samo.png";
			$arPicture = CFile::MakeFileArray($logo);
			$arFields['LOGOTIP'] = $arPicture;
		}
...
COption::SetOptionString("sotbit.b2bshop", "wizard_installed", "Y", false, WIZARD_SITE_ID);
?>