...
if( $arResult['ITEMS'] )
{
	...
	foreach($arResult['ITEMS'] as $idItem => $Item)
	{
		...
		if( $arParams['COLOR_IN_PRODUCT'] == 'Y' && $arParams['COLOR_IN_PRODUCT_CODE'] )
		{
			if( $Item["PROPERTIES"][$colorCode]["VALUE"] )
			{
				...
				{
					$PropArray = B2BSSotbitShop::GetValuesFromHL( $Item['PROPERTIES'][$colorCode]['USER_TYPE_SETTINGS']['TABLE_NAME'],
		...
		if( $Item['OFFERS'] )
		{
			for($j = 0; $j < sizeof( $Item['OFFERS'] ); ++$j)
			{
				...
				// set min price from offers
					$arResult["ITEMS"][$idItem]['MIN_PRICE'] = B2BSSotbitShop::GetMinPrice( $Item['MIN_PRICE'],
		...
		else
		{
			...
			if( $imageFromOffer )
			{
				$arResult["MORE_PHOTO_JS"][$idItem][0] = B2BSSotbitShop::GetImagesIfOffers( $Item["PREVIEW_PICTURE"],
						...
						$arParams['AJAX_PRODUCT_LOAD'], $arResult["MORE_PHOTO_JS"][$idItem][0], $Item['FIRST_COLOR'], 0 );
			}
		}
		...
		if( !$imageFromOffer )
		{
			...
			$ImageFromProduct = B2BSSotbitShop::ImagesFromProduct( $DetailPhoto, $Item['PROPERTIES']["MORE_PHOTO"]['VALUE'],
			...
			if(count($arResult["MORE_PHOTO_JS"][$idItem]) > 0 && !in_array
				($arResult["ITEMS"][$idItem]['FIRST_COLOR'],array_keys($arResult["MORE_PHOTO_JS"][$idItem])))
			{
		...
		{
			$arResult["MORE_PHOTO_JS"][$idItem] = B2BSSotbitShop::reOneColor( $arResult["MORE_PHOTO_JS"][$idItem], $arParams["LIST_WIDTH_SMALL"],
		...
		$SortPropertyTree = B2BSSotbitShop::SortPropertyTree( $arResult["ITEMS"][$idItem]["OFFER_TREE_PROPS_VALUE"], $colorCode, '', array(), array());
	...
	if( $arParams['COLOR_IN_PRODUCT'] == 'Y' && $arParams["COLOR_IN_SECTION_LINK"] == 2 && $arParams["COLOR_IN_PRODUCT_LINK"] )
	{
		...
		{
			$ColorLink2 = B2BSSotbitShop::ColorLink2( $arModelsLinks, $arParams["COLOR_IN_PRODUCT_LINK"], $arParams['COLOR_IN_PRODUCT_CODE'], $colorCode,
