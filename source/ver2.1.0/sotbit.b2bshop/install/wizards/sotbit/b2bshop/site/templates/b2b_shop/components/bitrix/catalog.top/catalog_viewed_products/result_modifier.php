...
for($i = 0; $i < $cntItems; ++$i)
{
	...
	if( $arParams['COLOR_IN_PRODUCT'] == 'Y' && $arParams['COLOR_IN_PRODUCT_CODE'] )
	{
		if( $arResult["ITEMS"][$i]["PROPERTIES"][$colorCode]["VALUE"] )
		{
			...
			{
				$PropArray = B2BSSotbitShop::GetValuesFromHL( $arResult["ITEMS"][$i]['PROPERTIES'][$colorCode]['USER_TYPE_SETTINGS']['TABLE_NAME'],
	...
	if( $arResult["ITEMS"][$i]['OFFERS'] )
	{
		for($j = 0; $j < sizeof( $arResult["ITEMS"][$i]['OFFERS'] ); ++$j)
		{
			...
			$arResult["ITEMS"][$i]['MIN_PRICE'] = B2BSSotbitShop::GetMinPrice( $arResult["ITEMS"][$i]['MIN_PRICE'],
	...
	else
	{
		...
		if( $imageFromOffer )
		{
			$arResult["MORE_PHOTO_JS"][$IdItem][0] = B2BSSotbitShop::GetImagesIfOffers( $arResult["ITEMS"][$i]["PREVIEW_PICTURE"],
	...
	if( !$imageFromOffer )
	{
		...
		$ImageFromProduct = B2BSSotbitShop::ImagesFromProduct( $DetailPhoto, $arResult["ITEMS"][$i]['PROPERTIES']["MORE_PHOTO"]['VALUE'],
