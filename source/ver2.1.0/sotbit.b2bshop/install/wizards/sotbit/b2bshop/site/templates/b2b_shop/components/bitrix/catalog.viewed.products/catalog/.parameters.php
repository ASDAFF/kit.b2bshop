...
$arIBlock = array();
$rsIBlock = CIBlock::GetList(array("sort" => "asc"), array("TYPE" => $arCurrentValues["OBJECTIVE_IBLOCK_TYPE"], "ACTIVE"=>"Y"));
while($arr=$rsIBlock->Fetch())
	$arIBlock[$arr["ID"]] = "[".$arr["ID"]."] ".$arr["NAME"];

$arTemplateParameters['OBJECTIVE_IBLOCK_TYPE'] = array(
			'PARENT' => 'MISS_SHOP',
			"NAME" => GetMessage("MS_TPL_IBLOCK_TYPE"),
			"TYPE" => "LIST",
			"VALUES" => $arIBlockType,
			"REFRESH" => "Y",
);

$arTemplateParameters['OBJECTIVE_IBLOCK_ID'] = array(
			"PARENT" => "MISS_SHOP",
			"NAME" => GetMessage("MS_TPL_IBLOCK_IBLOCK"),
			"TYPE" => "LIST",
			"ADDITIONAL_VALUES" => "Y",
			"VALUES" => $arIBlock,
			"REFRESH" => "Y",
...
$arSKU = false;
$boolSKU = false;
if ($boolCatalog && (isset($arCurrentValues['OBJECTIVE_IBLOCK_ID']) && 0 < intval($arCurrentValues['OBJECTIVE_IBLOCK_ID'])))
{
	$arSKU = CCatalogSKU::GetInfoByProductIBlock($arCurrentValues['OBJECTIVE_IBLOCK_ID']);
	$boolSKU = !empty($arSKU) && is_array($arSKU);
}

if (isset($arCurrentValues['OBJECTIVE_IBLOCK_ID']) && 0 < intval($arCurrentValues['OBJECTIVE_IBLOCK_ID']))
{
	$arAllPropList = array();
	...
	$rsProps = CIBlockProperty::GetList(
		...
		array('IBLOCK_ID' => $arCurrentValues['OBJECTIVE_IBLOCK_ID'], 'ACTIVE' => 'Y')
	);
...
$arTemplateParameters['DETAIL_PROPERTY_CODE'] = array(
            'PARENT' => 'MISS_SHOP',
            'NAME' => GetMessage('MS_TPL_DETAIL_PROPERTY_CODE'),
            'TYPE' => 'LIST',
            'MULTIPLE' => 'Y',
            'ADDITIONAL_VALUES' => 'N',
            'REFRESH' => 'N',
