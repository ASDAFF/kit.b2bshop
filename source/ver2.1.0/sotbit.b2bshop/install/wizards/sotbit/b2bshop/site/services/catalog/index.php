...
if( !in_array( $catalogView, array(
	...
	$catalogView = "list";
COption::SetOptionString( "sotbit.b2bshop", "catalogView", $catalogView, false, WIZARD_SITE_ID );
...
if( $module == 'sotbit.b2bshop' )
{
	...
	$personTypeFiz = COption::SetOptionString( "sotbit.b2bshop", "personTypeFiz", '', false, WIZARD_SITE_ID );
	$personTypeUr = COption::SetOptionString( "sotbit.b2bshop", "personTypeUr", '', false, WIZARD_SITE_ID );
	$personTypeIp = COption::SetOptionString( "sotbit.b2bshop", "personTypeIp", '', false, WIZARD_SITE_ID );

	if( $personTypeUr == 'Y' || $personTypeIp == 'Y' )
	{
		$iblockId = COption::GetOptionString( "sotbit.b2bshop", "IBLOCK_ID", "" );
		$iblockType = COption::GetOptionString( "sotbit.b2bshop", "IBLOCK_TYPE", "" );
		if( $iblockId > 0 )
		{
			COption::SetOptionString( "sotbit.b2bshop", "OPT_IBLOCK_ID", $iblockId );
			COption::SetOptionString( "sotbit.b2bshop", "OPT_IBLOCK_TYPE", $iblockType );
			...
			COption::SetOptionString( "sotbit.b2bshop", "OPT_BLANK_GROUPS", serialize( array(
					1
			) ) );
			...
			while ( $prop_fields = $properties->GetNext() )
			{
				...
				{
					COption::SetOptionString( "sotbit.b2bshop", "OPT_ARTICUL_PROP", $prop_fields['ID'] );
					break;
				}
			}
			...
			if( is_array( $mxResult ) && $mxResult['IBLOCK_ID'] > 0 )
			{
				...
				while ( $prop_fields = $properties->GetNext() )
				{
					...
					if( $prop_fields["CODE"] == 'CML2_ARTICLE' )
					{
						COption::SetOptionString( "sotbit.b2bshop", "OPT_ARTICUL_PROP_OFFER", $prop_fields['ID'] );
					}
				}
			...
			COption::SetOptionString( "sotbit.b2bshop", "OPT_PROPS", serialize( $props ) );

			COption::SetOptionString( "sotbit.b2bshop", "OPT_FILTER_FIELDS", serialize( $optFilterFields ) );
		}
	}
	...
	$optGroups = unserialize( COption::GetOptionString( "sotbit.b2bshop", "OPT_BLANK_GROUPS", "" ) );
	...
	COption::SetOptionString( "sotbit.b2bshop", "OPT_BLANK_GROUPS", serialize( $optGroups ) );
	...
	while ( $Site = $result->fetch() )
	{
		if( $OPT1_GROUP_ID > 0 )
		{
			COption::SetOptionString( "sotbit.auth", "WHOLESALERS_GROUP", $OPT1_GROUP_ID, '', $Site['LID'] );
		}
	}
	...
	COption::SetOptionString( "sotbit.b2bshop", "OPT_REGISTER_ORDER_FIELDS", serialize( $props ) );

	COption::SetOptionString( "sotbit.b2bshop", "OPT_REGISTER_FIELDS", serialize( array(
			'EMAIL',
			'NAME',
			'LAST_NAME'
	) ) );
	...
	COption::SetOptionString( "sotbit.b2bshop", "PRICE_CODE", serialize( $prices ) );
}
// ///////
if( COption::GetOptionString( "sotbit.b2bshop", "wizard_installed", "N", WIZARD_SITE_ID ) == "Y" && !WIZARD_INSTALL_DEMO_DATA )
	return;
