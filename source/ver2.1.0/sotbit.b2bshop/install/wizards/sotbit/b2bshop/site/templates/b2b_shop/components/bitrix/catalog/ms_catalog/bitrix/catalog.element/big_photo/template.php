		...
		<div class="detail_big_pic" data-rnd="<?=$arResult["RAND"]?>">
			...
			<?if(count($arResult["MORE_PHOTO_JS"][$firstColor]["MEDIUM"])>0)
			{
				...
				foreach($arResult["MORE_PHOTO_JS"][$firstColor]["MEDIUM"] as $i=>$arMediumImg)
				{
					...
					?>
					<a class="big_foto item" href="<?=$arBigImg["src"]?>" onclick="return false;" title="" style="display:<?if($firstImg):?>block<?else:?>none<?endif;?>">
...
<div class="col-sm-12">
	...
	<div class="row">
		<div class="col-sm-24 sm-padding-no">
			<div class="detail_block_buy">
				<div class="row">
							...
							<div class="discount-block_price-inner">
								<div class="detail_block_price available-container">
									...
									<?if($arResult["PRICES_JS"][$FirstOffer]['OLD_PRICE'] && $arResult["PRICES_JS"][$FirstOffer]['OLD_PRICE']!=$arResult["PRICES_JS"][$FirstOffer]['DISCOUNT_PRICE']):?>
										<div class="wrapper-old-price">
											<span class="old_price">
												<?=$arResult["PRICES_JS"][$FirstOffer]['OLD_PRICE']?>
											</span>
					...
					<div class="available-container">
						<div class="block-right-column">
							...
							<div class="available-block available-block-true">
								...
								<?=GetMessage("B2BS_CATALOG_PRODUCT_AVAILABLE")?>
							</div>
						</div>
							<?
							...
							if($isPhone=="Y"):
								?>
								<?$APPLICATION->IncludeComponent("sotbit:order.phone", "catalog", array(
					...
					{?>
						<div class="subscribe_cont block-right-column" <?=(isset($arResult["OFFER_SUBSCRIBE_URL"][$FirstOffer]))?'style="display:block"':'style="display:none"'?>>
							<?
		...
		<?} ?>
<div class="row detail-analog-product detail-analog-product-under">
			<div class="col-sm-12 sm-padding-left-no">
			<?$frame = $this->createFrame()->begin();?>
				<?
				...
				if(\Bitrix\Main\Config\Option::get("sotbit.b2bshop", "SHOW_LOOKING_ELEMENT","Y") == 'Y')
				{
					$APPLICATION->IncludeComponent("sotbit:product.view", "ms_vertical", array(
				...
				if(\Bitrix\Main\Config\Option::get("sotbit.b2bshop", "SHOW_ANALOG_PRODUCTS_ELEMENT","Y") == 'Y')
				{
					$APPLICATION->IncludeComponent("sotbit:analog.products", "ms_vertical", array(
						"IBLOCK_TYPE" => $arParams["IBLOCK_TYPE"],
						"IBLOCK_ID" => $arParams["IBLOCK_ID"],
						"ID" => $arResult["ID"],
						"PROPERTY_CODE" => $arParams["PROPERTY_CODE"],
				...
				if($sMSRightBlockCount<2 && \Bitrix\Main\Config\Option::get("sotbit.b2bshop", "SHOW_BRAND_PRODUCT_ELEMENT","Y") == 'Y')
				{
					if($sMSRightBlockCount == 0)
					{
				...
				$context = \Bitrix\Main\Application::getInstance()->getContext();
				$server = $context->getServer();
				if(\Bitrix\Main\Config\Option::get("sotbit.b2bshop", "SHOW_BIG_DATA_ELEMENT_RIGHT","Y") == 'Y' && \Bitrix\Main\Config\Option::get("main", "gather_catalog_stat","N") == 'Y' && is_dir($server->getDocumentRoot().$server->getPersonalRoot().'/components/bitrix/catalog.bigdata.products'))
				{
					if($sMSRightBlockCount == 1)
					{
					?>
						</div>
						<div class="col-sm-12 sm-padding-right-no">
					...
					if($sMSRightBlockCount == 2)
					{
						?>
						</div>
							<div class="col-sm-12 sm-padding-left-no detail-analog-product detail-analog-product-under">
						<?php
					}
