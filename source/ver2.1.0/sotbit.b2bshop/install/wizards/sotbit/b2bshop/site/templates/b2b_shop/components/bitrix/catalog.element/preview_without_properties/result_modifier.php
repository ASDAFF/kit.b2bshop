...
if (Bitrix\Main\Loader::includeModule( "sotbit.regions" ) && function_exists('\Sotbit\Regions\Sale\Price::change'))
{
		...
		if( $arParams['OFFER_TREE_PROPS'] )
		{
			...
			for($k = 0; $k < sizeof( $arParams['OFFER_TREE_PROPS'] ); ++$k)
			{
				if( $arResult['OFFERS'][$j]['PROPERTIES'][$arParams['OFFER_TREE_PROPS'][$k]]['VALUE'] && $arResult['OFFERS'][$j]['PROPERTIES'][$arParams['OFFER_TREE_PROPS'][$k]]["USER_TYPE"] == 'directory' )
				{
					...
					{
						$PropArray = B2BSSotbitShop::GetValuesFromHL( $arResult['OFFERS'][$j]['PROPERTIES'][$arParams['OFFER_TREE_PROPS'][$k]]['USER_TYPE_SETTINGS']['TABLE_NAME'], $arSKU['IBLOCK_ID'],
		...
		if( $arOfferParams )
		{
			for($k = 0; $k < sizeof( $arOfferParams ); ++$k)
			{
				if( $arResult['OFFERS'][$j]['PROPERTIES'][$arOfferParams[$k]] )
				{
					...
					{
						$PropArray = B2BSSotbitShop::GetValuesFromHL( $arResult['OFFERS'][$j]['PROPERTIES'][$arOfferParams[$k]]['USER_TYPE_SETTINGS']['TABLE_NAME'], $arSKU['IBLOCK_ID'],
								$arOfferParams[$k] );
						$HLParams[key( $PropArray )] = $PropArray[key( $PropArray )];
						unset( $PropArray );
					}
...
if( !$imageFromOffer )
{
	...
	$ImageFromProduct = B2BSSotbitShop::ImagesFromProduct( $DetailPhoto, $arResult["MORE_PHOTO"], $HLParams[$colorCode], $arResult["FIRST_COLOR"], $arParams['COLOR_IN_PRODUCT'],
