...
require_once($_SERVER["DOCUMENT_ROOT"]."/bitrix/wizards/sotbit/b2bshop/WizardHelper.php");
...
class InstallDateWizards extends CWizardStep {
    function InitStep()
    {
        ...
        $wizard->solutionName = "sotbit.b2bshop";
    ...
    function OnPostForm()
    {
        if (CModule::IncludeModule('sotbit.b2bshop') === false) {
...
class InstallOrigami extends CWizardStep
{
    ...
    function OnPostForm()
    {
        ...
        header('Location: ' . $protocol . '://' . $_SERVER['HTTP_HOST'] . '/bitrix/admin/wizard_install.php?' . bitrix_sessid_get() . '&lang=' . LANGUAGE_ID . '&wizardName=sotbit:origami');
    }
