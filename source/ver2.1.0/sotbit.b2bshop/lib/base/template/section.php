...
class Section
{
	...
	public function identifySectionView($sectionLevel = 0)
	{
		...
		$viewCookie = Application::getInstance()->getContext()->getRequest()->getCookie("sotbit_section_view");;

		if($view && in_array($view, $this->views))
		{
			$cookie = new \Bitrix\Main\Web\Cookie("sotbit_section_view", $view);
		...
		{
			$this->setSectionView(\Bitrix\Main\Config\Option::get('sotbit.b2bshop','SECTION_VIEW','block'));
		}

		if( \Bitrix\Main\Config\Option::get("sotbit.b2bshop","SHOW_BRICKS","N") == 'Y '&& $sectionLevel == 1)
		{
