...
class CModuleOptions
{
	...
	private function SaveOptions()
	{
		...
		{
			if($opt=='ADD_MENU_LINKS')
			{
				$ValArray = array ();

				if(is_array( $_POST['ADD_MENU_LINKS_PARENT_LINK'] ) && is_array( $_POST['ADD_MENU_LINKS_TITLE'] ) && is_array( $_POST['ADD_MENU_LINKS_URL'] ) && is_array( $_POST['ADD_MENU_LINKS_SORT'] ))
				{
					foreach( $_POST['ADD_MENU_LINKS_PARENT_LINK'] as $i => $val )
					{
						$ValArray[$i]['ADD_MENU_LINKS_PARENT_LINK'] = $val;
						$ValArray[$i]['ADD_MENU_LINKS_TITLE'] = $_POST['ADD_MENU_LINKS_TITLE'][$i];
						$ValArray[$i]['ADD_MENU_LINKS_URL'] = $_POST['ADD_MENU_LINKS_URL'][$i];
						$ValArray[$i]['ADD_MENU_LINKS_SORT'] = $_POST['ADD_MENU_LINKS_SORT'][$i];
					}
					...
					if(is_array( $ValArray ))
					{
						...
						$strSqlWhere = sprintf(
								...
								$sqlHelper->forSql($this->module_id),
								$sqlHelper->forSql($opt)
								);
			...
			if($arOptParams['TYPE']!='CUSTOM')
			{
				...
				{
					COption::SetOptionString( $this->module_id, $opt.'_OTHER', $_REQUEST[$opt.'_OTHER'] );
				}
