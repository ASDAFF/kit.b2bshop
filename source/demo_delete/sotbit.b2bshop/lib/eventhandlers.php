...
class EventHandlers {
    ...
    public function onBeforeResultAdd($WEB_FORM_ID, &$arFields, &$arrVALUES) {
        if($WEB_FORM_ID == \COption::GetOptionString("sotbit.b2bshop", "FORM_MANAGER_CALLBACK", false)) {
            //\CEvent::Send("SOTBIT_AUTH_NEW_USER_PASSWORD", SITE_ID, array_merge($arEventFields, $arFields));
            $keyName = \COption::GetOptionString("sotbit.b2bshop", "FORM_FIELD_NAME", false);
            $keyPhone = \COption::GetOptionString("sotbit.b2bshop", "FORM_FIELD_PHONE", false);
            $keyComment = \COption::GetOptionString("sotbit.b2bshop", "FORM_FIELD_COMMENT", false);
            $arrayFields = array();
            if($keyName)
