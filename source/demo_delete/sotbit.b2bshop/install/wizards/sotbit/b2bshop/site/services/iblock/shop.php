...
$iblockXMLFile = WIZARD_SERVICE_RELATIVE_PATH."/xml/ru/ms_shop.xml"; 
$iblockCode = "ms_contact_".WIZARD_SITE_ID;
$iblockType = "b2bs_content"; 

$rsIBlock = CIBlock::GetList(array(), array("XML_ID" => $iblockCode, "TYPE" => $iblockType));
$IBLOCK_SHOP_ID = false; 
if ($arIBlock = $rsIBlock->Fetch())
{
    $IBLOCK_SHOP_ID = $arIBlock["ID"]; 
...
if($IBLOCK_SHOP_ID == false)
{
    ...
    $IBLOCK_SHOP_ID = WizardServices::ImportIBlockFromXML(
        $iblockXMLFile,
        "contact",
        $iblockType,
        WIZARD_SITE_ID,
    ...
    $iblock->Update($IBLOCK_SHOP_ID, $arFields);
    $_SESSION["WIZARD_SHOP_IBLOCK_ID"] = $IBLOCK_SHOP_ID;
}
else
{
    $arSites = array(); 
    $db_res = CIBlock::GetSite($IBLOCK_SHOP_ID);
    ...
    if (!in_array(WIZARD_SITE_ID, $arSites))
    {
        ...
        $iblock = new CIBlock;
        $iblock->Update($IBLOCK_SHOP_ID, array("LID" => $arSites));
        $_SESSION["WIZARD_SHOP_IBLOCK_ID"] = $IBLOCK_SHOP_ID;
    }
}

CWizardUtil::ReplaceMacros(WIZARD_SITE_PATH."/about/contacts/index.php", array("SHOP_IBLOCK_ID" => $IBLOCK_SHOP_ID));
?>