...
$iblockXMLFile = WIZARD_SERVICE_RELATIVE_PATH."/xml/ru/ms_news.xml"; 
...
if($IBLOCK_NEWS_ID == false)
{
	...
	$IBLOCK_NEWS_ID = WizardServices::ImportIBlockFromXML(
		$iblockXMLFile,
		"news",
		$iblockType,
		WIZARD_SITE_ID,
	...
	$iblock->Update($IBLOCK_NEWS_ID, $arFields);
    $_SESSION["WIZARD_NEWS_IBLOCK_ID"] = $IBLOCK_NEWS_ID;
}
else
{
	$arSites = array(); 
	$db_res = CIBlock::GetSite($IBLOCK_NEWS_ID);
	...
	if (!in_array(WIZARD_SITE_ID, $arSites))
	{
		...
		$iblock = new CIBlock;
		$iblock->Update($IBLOCK_NEWS_ID, array("LID" => $arSites));
        $_SESSION["WIZARD_NEWS_IBLOCK_ID"] = $IBLOCK_NEWS_ID;
	}
}


CWizardUtil::ReplaceMacros($_SERVER["DOCUMENT_ROOT"].BX_PERSONAL_ROOT."/templates/".WIZARD_TEMPLATE_ID.'/header.php' , array("NEWS_IBLOCK_ID" => $IBLOCK_NEWS_ID)); 
CWizardUtil::ReplaceMacros(WIZARD_SITE_PATH."/news/index.php", array("NEWS_IBLOCK_ID" => $IBLOCK_NEWS_ID));


if($IBLOCK_NEWS_ID){
    COption::SetOptionString("sotbit.b2bshop",'NEWS_IBLOCK_TYPE',$iblockType);      
    COption::SetOptionString("sotbit.b2bshop",'NEWS_IBLOCK_ID',$IBLOCK_NEWS_ID);    
} 
