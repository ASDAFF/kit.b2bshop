    ...
    <?if($arParams["SHOW_ORDER_BASKET"]=='Y'):?>
        <table class="bx_order_list_table_order" width="100%" cellpadding="0" cellspacing="0" border="0" style="border: 1px solid #d3d3d3;margin:0; padding:0px;">
            ...
            <tr>
                ...
                endforeach;
                ?>
                <td <?=$colspan?> align="center"><span style="font: 13px 'Proxima Nova', Arial, sans-serif;color: #999999;line-height:20px;padding: 12px 0;display: block; -webkit-text-size-adjust:none;"><?=GetMessage('SPOD_PROPERTY_TOTAL')?></span></td>
            </tr>
            </thead>
            <tbody>
            <? $i_color = 2;
            foreach($arResult["BASKET"] as $prod):
                ...
                endforeach;
                ?>
                <td class="custom" align="<?=$align?>" <?=$width?>><span style="font: 13px 'Proxima Nova', Arial, sans-serif;color: #000000;padding: 12px 0;text-decoration: none;line-height:24px;display: block; -webkit-text-size-adjust:none;"><?=$prod['TOTAL_PRICE']?></span></td>
                </tr><?
                $i_color++;
            endforeach;
            ?>
