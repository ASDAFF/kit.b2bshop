...
else
{
	...
	?>
	<div class="personal-wrapper personal-reviews-wrapper personal-reviews-wrapper-fiz <?=($opt->hasAccess())?'personal-wrapper-access':''?>">
		<?php
		$Template = new \Sotbit\B2BShop\Client\Template\Main();
		$Template->includeBlock('template/personal/tabs.php');
		?>

		<div class="row border-profile border-profile-reviews border-profile-reviews-fiz">
			<?
			...
			<div class="col-sm-19 sm-padding-right-no blank_right-side <?= (!$menu->isOpen()) ? 'blank_right-side_full' : '' ?>" id="blank_right_side">
				<div id="wrapper_blank_resizer" class="wrapper_blank_resizer">
					...
					<div class="personal-right-content">
						<?
						if(file_exists($_SERVER['DOCUMENT_ROOT'] . '/bitrix/components/sotbit/reviews.personalreviews/component.php'))
						{
							$APPLICATION->IncludeComponent(
								"sotbit:reviews.personalreviews",
								"bootstrap",
								array(
									'MAX_RATING' => "5",
