...
if($module == 'sotbit.b2bshop')
{
	...
	if($UrPersonTypes)
	{
		Option::set("sotbit.b2bshop", "BUYER_PERSONAL_TYPE", serialize($UrPersonTypes));
		Option::set("sotbit.b2bshop", "SHOW_PERSON_TYPE_BUYERS", serialize($UrPersonTypes));
	}
	Option::set("sotbit.b2bshop", "SECTION_VIEW_ACCESS", serialize(array('block','row')));
	Option::set("sotbit.b2bshop", "OPT_PERSONAL_ORDER_ACTIONS", serialize(array('repeat','cancel')));
	...
	Option::set("sotbit.b2bshop",'PRICE_CODE_SECTION',serialize($prices));
	if($UrPersonTypes)
	{
	...
	if($propsInn)
	{
		\Bitrix\Main\Config\Option::set('sotbit.b2bshop', 'DOCUMENT_ORG', serialize($propsInn));
	}
	if($propsCompany)
	{
		\Bitrix\Main\Config\Option::set('sotbit.b2bshop', 'DOCUMENT_ORG_NAME', serialize($propsCompany));
	}
	...
	if($personTypes)
	{
		...
		$iblock = \Bitrix\Main\Config\Option::get('sotbit.b2bshop','DOCUMENT_IBLOCK_ID');
		$els = [];
