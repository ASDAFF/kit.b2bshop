				...
				<div id="item_<?=$arItem["ID"]?>" class="basket_item <?=$color?>">
					...
					<div class='div_table__delete'>
						<div class="wrap_del_change">
							<p class="cart-delay-item"><a title="<?=GetMessage("MS_BASKET_DO_DELAY")?>" href="<?=str_replace("#ID#", $arItem["PRODUCT_ID"], $arUrls["new_delay"])?>" onclick="BX.ajax.insertToNode('<?=str_replace("#ID#", $arItem["PRODUCT_ID"], $arUrls["new_delay"])?>&bxajaxid=<?=$bxajaxid?>', 'comp_<?=$bxajaxid;?>'); return false;" rel="nofollow"></a></p>
                            <p class="cart-delete-item"><a title="<?=GetMessage("MS_BASKET_DO_DELAY")?>" href="<?=str_replace("#ID#", $arItem["ID"], $arUrls["delete"])?>"   rel="nofollow"></a></p>
						</div>
					</div>
				</div>
				<? $kk++; ?>
