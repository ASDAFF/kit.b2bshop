...
WizardServices::IncludeServiceLang("step3.php", 'ru');    
...
{ 
    ...
    {           
        $arEventSend[] = 'SALE_STATUS_CHANGED_'.$arStatus['ID'];            
    }        
}
$dbEvent = CEventMessage::GetList($b="ID", $order="ASC", Array("SITE_ID" => WIZARD_SITE_ID));
while($arEvent = $dbEvent->Fetch())
{ 
    if(in_array($arEvent['EVENT_NAME'],$arEventSend)){ 
...
if(!($dbEvent->Fetch()))
{
	...
	$emess->Add(array(
		...
		"EMAIL_TO" => "#EMAIL#",
		"BCC" => "#BCC#",
		"SUBJECT" => GetMessage("SALE_SUBSCRIBE_PRODUCT_SUBJECT"),
		"MESSAGE" => GetMessage("SALE_SUBSCRIBE_PRODUCT_MESSAGE"),
		"BODY_TYPE" => "html",
	));
