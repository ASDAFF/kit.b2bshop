...
if (sizeof( $arResult['ITEMS'] ) > 0)
{
	...
	$arResult["IS_DOWNLOAD"] = COption::GetOptionString( "sotbit.b2bshop", "DOWNLOAD", "" );
	$arMainProps = unserialize( COption::GetOptionString( "sotbit.b2bshop", "MAIN_PROPS", "" ) );
	
	$arResult["SIZE_PROPS"] = $arSizeProps = unserialize( COption::GetOptionString( "sotbit.b2bshop", "SIZE_PROPS", "" ) );
	// START LINK IN PROPS
	$FilterProps = array ();
	...
	{
		if (isset( $colorCode ) && ! empty( $colorCode ) && ! in_array( $colorCode, reset( $arParams['OFFER_TREE_PROPS'] ) ))
		{
			array_push( reset( $arParams['OFFER_TREE_PROPS'] ), $colorCode );
		}
	...
	foreach ( $arResult['ITEMS'] as $i => $Item )
	{
			...
			if (isset( $arColorXml[$ID] ))
			{
				foreach ( $arColorXml[$ID] as $xmlID => $arV )
				{
					if (! isset( $arResult["MORE_PHOTO_JS"][$ID][$xmlID] ))
						$arResult["MORE_PHOTO_JS"][$ID][$xmlID] = $Item["DEFAULT_IMAGE"];
				}
			}
		}
		else
		...
		// START OFFER_PROPS
		$arOfferProps = unserialize( COption::GetOptionString( "sotbit.b2bshop", "OFFER_ELEMENT_PROPS", "" ) );
		$arOfferParams = unserialize( COption::GetOptionString( "sotbit.b2bshop", "OFFER_ELEMENT_PARAMS", "" ) );
		// END OFFER_PROPS
		...
		if (isset( $arResult["OFFER_TREE_PROPS"][$ID] ) && is_array( $arResult["OFFER_TREE_PROPS"][$ID] ))
			...
			:
				...
				if ($colorCode == $codeProp || ! in_array( $codeProp, unserialize( COption::GetOptionString( "sotbit.b2bshop", "OFFER_TREE_PROPS", "" ) ) ))
					continue;
