...
<div id="block_filter_js" class="block_form_filter" data-site-url="<?=$APPLICATION->GetCurPage()?>" data-site-dir="<?=$_SERVER['DOCUMENT_ROOT'].SITE_DIR?>">
	<form name="<?echo $arResult["FILTER_NAME"]."_form"?>" action="<?echo $arResult["FORM_ACTION"]?>" method="get" class="smartfilter">
		...
		<?switch ($arItem["DISPLAY_TYPE"])
		{
			...
			?>
			<div class="filter_block n filter_color <?=(isset($arResult['OPENS'][$arItem['CODE']]) && $arResult['OPENS'][$arItem['CODE']]=='Y')?'block_open':'' ?>" data-code="<?=$arItem["CODE"]?>">
			...
			<div class="filter_block <?=(isset($arResult['OPENS'][$arItem['CODE']]) && $arResult['OPENS'][$arItem['CODE']]=='Y')?'block_open':'' ?>" data-code="<?=$arItem["CODE"]?>">
				<span class="block_name" onclick="open_close_filter(this, '.inner_filter_block', '.filter_block');"><?=$arItem["NAME"]?></span>
				<div class="inner_filter_block <?if(count($arItem["VALUES"]) > $defOpenLi):?>scrollbarY<?endif;?>" style="<?php echo (isset($arResult['OPENS'][$arItem['CODE']]) && $arResult['OPENS'][$arItem['CODE']]=='Y')?'display:block;':'display:none;';?> <?php echo (count($arItem["VALUES"]) > $defOpenLi)?'height: '.$defLiHight.'px;':'';?>" >
						<ul class="overview">
						<?$indexValue = 0;?>
