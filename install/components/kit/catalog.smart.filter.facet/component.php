			...
			if(isset($_REQUEST["ajax"]) && $_REQUEST["ajax"] === "y")
				...
				$_CHECK = $_SESSION[$FILTER_NAME][$this->SECTION_ID];
					...
					if($pars)
					{
						...
						{
							if(array_keys($par) == array('from','to'))
							{
								foreach($par as $pa)
								{
									if($pa > 0)
					...
					if($s404 && (!$hasChecked || $s404Cnt != $hasCheckedCnt) && empty($_CHECK['q']))
					{
						if(($_SESSION['MS_ONLY_CHECKED'] == 'Y' || $_SESSION['MS_SECTIONS'] || $_SESSION['MS_ONLY_AVAILABLE'] == 'Y' || $_SESSION['MS_COUNT'] > 0 || $_SESSION['MS_SORT'] > 0) && !$_CHECK)
						{
					...
					else
					{
						...
						elseif($arParams["SEF_MODE_FILTER"]=="Y")
						{
							...
							if(class_exists('B2BSKitParent'))
							{
								$APPLICATION->SetCurPage(B2BSKitParent::$pageURL, "");
							}elseif(class_exists('B2BSKitParent'))
							{
								$APPLICATION->SetCurPage(B2BSKitParent::$pageURL, "");
							}
						}
