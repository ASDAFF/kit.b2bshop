...
if($_REQUEST['CATEGORIES_ID_CHECK']){
    $_REQUEST['CATEGORIES_ID'] = $_REQUEST['CATEGORIES_ID_CHECK'];        
}  
if(empty($_REQUEST['CATEGORIES_ID_CHECK']) && $_REQUEST['SUBSCRIBER_ID']){
    $_REQUEST['CATEGORIES_ID'] = array('0');        
}      
...
if($_REQUEST['getemail']=='Y' && !empty($_REQUEST['EMAIL_TO'])){
    ...
    if(in_array($_REQUEST['PARAM_1'],array('SECTION_ID','PROPERTY'))){
        ...
        if($arfillterCat) {
            // ������� ��������� ��������
            $new_category_id = '';
            $sel = CKitMailingCategories::GetList(array(),$arfillterCat ,false,array('ID','PARAM_1','PARAM_2','PARAM_3'));
            while($res = $sel->Fetch()){
            ...
            if(empty($new_category_id)) {
                ...
                $new_category_id = CKitMailingCategories::Add($arrNewCategory);
                global $CACHE_MANAGER; 
                $CACHE_MANAGER->ClearByTag('kit.mailing_GetCategoriesInfo');
    ...
    if($_REQUEST['SUBSCRIBER_ID']){   
        $UPDATE_EMAIL_SAVE = array('EMAIL_TO'=>$EMAIL_SAVE['EMAIL_TO'],'CATEGORIES_ID'=>$EMAIL_SAVE['CATEGORIES_ID']); 
        CKitMailingSubscribers::Update($_REQUEST['SUBSCRIBER_ID'], $UPDATE_EMAIL_SAVE);
    } 
    else {
        $ANSWER = CkitMailingSubTools::AddSubscribers($EMAIL_SAVE);
    }
