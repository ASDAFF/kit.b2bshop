			...
			<div id="item_<?=$arItem["ID"]?>" class="basket_item <?=$color?>">
				...
				<div class='div_table__delete'>
					<div class="wrap_del_change">
						<p class="cart-add-item"><a title="<?=GetMessage("MS_BASKET_TO_BASKET")?>" href="<?=str_replace("#ID#", $arItem["ID"], $arUrls["add"])?>&delay=1" rel="nofollow"></a></p>
                        <p class="cart-delete-item"><a title="<?=GetMessage("MS_BASKET_DELETE")?>" href="<?=str_replace("#ID#", $arItem["ID"], $arUrls["delete"])?>" rel="nofollow"></a></p>
					</div>
				</div>
			</div>
			<? $kk++; ?>
