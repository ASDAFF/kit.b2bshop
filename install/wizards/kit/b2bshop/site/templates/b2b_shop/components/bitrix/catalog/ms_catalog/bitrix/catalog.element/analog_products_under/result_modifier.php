...
if (Bitrix\Main\Loader::includeModule( "kit.regions" ) && class_exists('\Kit\Regions\Sale\Price'))
{
	$arResult = \Kit\Regions\Sale\Price::change( $arResult );
}

$arResult["IS_DOWNLOAD"] = \Bitrix\Main\Config\Option::get( "kit.b2bshop", "DOWNLOAD", "" );
$arMainProps = unserialize( \Bitrix\Main\Config\Option::get( "kit.b2bshop", "MAIN_PROPS", "" ) );
$arDopProps = unserialize( \Bitrix\Main\Config\Option::get( "kit.b2bshop", "DOP_PROPS", "" ) );
$arResult["SIZE_PROPS"] = unserialize( \Bitrix\Main\Config\Option::get( "kit.b2bshop", "SIZE_PROPS", "" ) );
$arResult["TABLE_SIZE_URL"] = \Bitrix\Main\Config\Option::get( "kit.b2bshop", "TABLE_SIZE_URL", "" );
$arOfferProps = unserialize( \Bitrix\Main\Config\Option::get( "kit.b2bshop", "OFFER_ELEMENT_PROPS", "" ) );
$arOfferParams = unserialize( \Bitrix\Main\Config\Option::get( "kit.b2bshop", "OFFER_ELEMENT_PARAMS", "" ) );
...
$arResult['COLOR_OFFER_FROM_IMAGE'] = unserialize(\Bitrix\Main\Config\Option::get( "kit.b2bshop", "COLOR_OFFER_FROM_IMAGE",	"" ));
...
if( $arResult['OFFERS'] )
{
	if( $arParams['COLOR_IN_PRODUCT'] == 'Y' && $arParams['COLOR_IN_PRODUCT_CODE'] && $arResult["PROPERTIES"][$arParams["COLOR_IN_PRODUCT_LINK"]]["VALUE"])
	{
		if( $arResult["PROPERTIES"][$colorCode]["VALUE"] )
		{
			...
			{
				$PropArray = B2BSKitShop::GetValuesFromHL( $arResult['PROPERTIES'][$colorCode]['USER_TYPE_SETTINGS']['TABLE_NAME'],
	...
	for($j = 0; $j < sizeof( $arResult['OFFERS'] ); ++$j)
	{
		...
		if( $arResult['OFFERS'][$j]["CAN_BUY"] )
		{
			...
			$arResult["OFFER_DELAY_URL"][$arResult['OFFERS'][$j]["ID"]] = str_replace( $arParams["ACTION_VARIABLE"] . "=ADD2BASKET", $arParams["ACTION_VARIABLE"] . "=DELAY",
					$arResult["OFFER_ADD_URL"][$arResult['OFFERS'][$j]["ID"]] );
		...
		if( $arParams['OFFER_TREE_PROPS'] )
		{
			...
			if( sizeof( $Tables ) > 0 )
			{
				$HLParams = B2BSKitShop::GetValuesFromHL( $Tables );
			}
			for($k = 0; $k < sizeof( $arParams['OFFER_TREE_PROPS'] ); ++$k)
			{
				if( $arResult['OFFERS'][$j]['PROPERTIES'][$arParams['OFFER_TREE_PROPS'][$k]]['VALUE'] && $arResult['OFFERS'][$j]['PROPERTIES'][$arParams['OFFER_TREE_PROPS'][$k]]["USER_TYPE"] == 'directory' )
				{
					...
					{
						$PropArray = B2BSKitShop::GetValuesFromHL( $arResult['OFFERS'][$j]['PROPERTIES'][$arParams['OFFER_TREE_PROPS'][$k]]['USER_TYPE_SETTINGS']['TABLE_NAME'], $arSKU['IBLOCK_ID'],
		...
		if( $arResult['OFFERS'][$j]["CAN_BUY"] )
		{
			$arResult['MIN_PRICE'] = B2BSKitShop::GetMinPrice( $arResult['MIN_PRICE'], $arResult['OFFERS'][$j]["MIN_PRICE"] );
		...
		if( $arOfferParams )
		{
			for($k = 0; $k < sizeof( $arOfferParams ); ++$k)
			{
				if( $arResult['OFFERS'][$j]['PROPERTIES'][$arOfferParams[$k]]['VALUE'] )
				{
					...
					{
						$PropArray = B2BSKitShop::GetValuesFromHL( $arResult['OFFERS'][$j]['PROPERTIES'][$arOfferParams[$k]]['USER_TYPE_SETTINGS']['TABLE_NAME'], $arSKU['IBLOCK_ID'],
								$arOfferParams[$k] );
						$HLParams[key( $PropArray )] = $PropArray[key( $PropArray )];
						unset( $PropArray );
					}
		...
		{
			$arResult["MORE_PHOTO_JS2"][$arResult['OFFERS'][$j]['ID']] = B2BSKitShop::GetImagesIfOffers( $arResult['OFFERS'][$j]["PREVIEW_PICTURE"], $arResult['OFFERS'][$j]["DETAIL_PICTURE"], $arResult['OFFERS'][$j]["PROPERTIES"][$arParams["MORE_PHOTO_OFFER_PROPS"]]["VALUE"],
		...
		{
			$arResult["MORE_PHOTO_JS"][$colorXmlID] = B2BSKitShop::GetImagesIfOffers( $arResult['OFFERS'][$j]["PREVIEW_PICTURE"], $arResult['OFFERS'][$j]["DETAIL_PICTURE"], $arResult['OFFERS'][$j]["PROPERTIES"][$arParams["MORE_PHOTO_OFFER_PROPS"]]["VALUE"],
				$arResult['PROPERTIES']['ADD_PICTURES']['VALUE'], $arResult["PREVIEW_PICTURE"], $arResult["DETAIL_PICTURE"], $arParams["DETAIL_WIDTH_SMALL"], $arParams["DETAIL_HEIGHT_SMALL"], $arParams["DETAIL_WIDTH_MEDIUM"], $arParams["DETAIL_HEIGHT_MEDIUM"], $arParams["DETAIL_WIDTH_BIG"],
...
else
{
	...
	if( $imageFromOffer )
	{
		if(count($arResult['COLOR_OFFER_FROM_IMAGE']) > 0)
		{
			$arResult["MORE_PHOTO_JS2"][$arResult['ID']] = B2BSKitShop::GetImagesIfOffers($arResult["PREVIEW_PICTURE"], $arResult["DETAIL_PICTURE"], $arResult["PROPERTIES"][$arParams["MORE_PHOTO_PRODUCT_PROPS"]]["VALUE"], $arResult['PROPERTIES']['ADD_PICTURES']['VALUE'], $arResult["PREVIEW_PICTURE"],
		...
		{
			$arResult["MORE_PHOTO_JS"][0] = B2BSKitShop::GetImagesIfOffers( $arResult["PREVIEW_PICTURE"], $arResult["DETAIL_PICTURE"], $arResult["PROPERTIES"][$arParams["MORE_PHOTO_PRODUCT_PROPS"]]["VALUE"], $arResult['PROPERTIES']['ADD_PICTURES']['VALUE'], $arResult["PREVIEW_PICTURE"],
		...
		if( $arResult["CAN_BUY_FIRST_OFFER"] )
		{
			...
			$arResult["OFFER_DELAY_URL"][$arResult['ID']] = str_replace( $arParams["ACTION_VARIABLE"] . "=ADD2BASKET", $arParams["ACTION_VARIABLE"] . "=DELAY",
					$arResult["OFFER_ADD_URL"][$arResult['ID']] );
	...
	// set microrazmetka
	$arResult['MICRORAZMETKA']['PRICE'] = ( int ) B2BSKit::ExpandStringWithMacros( COption::GetOptionString( "kit.b2bshop", "MICRO_TOVAR_PRICE", "" ), $arResult );
...
if( !$imageFromOffer )
{
	if( $arParams['COLOR_IN_PRODUCT'] == 'Y' && $arParams['COLOR_IN_PRODUCT_CODE'] )
	{
		if( $arResult["PROPERTIES"][$colorCode]["VALUE"] )
		{
			if( !isset( $HLParams[$colorCode] ) )
			{
				$PropArray = B2BSKitShop::GetValuesFromHL( $arResult["ITEMS"][$i]['PROPERTIES'][$colorCode]['USER_TYPE_SETTINGS']['TABLE_NAME'], $arResult['CATALOG']['IBLOCK_ID'], $colorCode );
				$HLParams[key( $PropArray )] = $PropArray[key( $PropArray )];
			}
	...
	else
	{
		...
		if( !isset( $HLParams[$colorCode] ) )
		{
			$PropArray = B2BSKitShop::GetValuesFromHL( $arResult['PROPERTIES'][$colorCode]['USER_TYPE_SETTINGS']['TABLE_NAME'], $arSKU['IBLOCK_ID'], $colorCode );
			$HLParams[key( $PropArray )] = $PropArray[key( $PropArray )];
		}
	}
	...
	$ImageFromProduct = B2BSKitShop::ImagesFromProduct( $DetailPhoto, $arResult["MORE_PHOTO"], $HLParams[$colorCode], $arResult["FIRST_COLOR"], $arParams['COLOR_IN_PRODUCT'],
			$arParams["COLOR_IN_SECTION_LINK"], 'N', $arParams["DETAIL_WIDTH_SMALL"], $arParams["DETAIL_HEIGHT_SMALL"], $arParams["DETAIL_WIDTH_MEDIUM"], $arParams["DETAIL_HEIGHT_MEDIUM"],
			$arParams["DETAIL_WIDTH_BIG"], $arParams["DETAIL_HEIGHT_BIG"], $ResizeMode, $arParams['OFFER_TREE_PROPS'], $colorCode );
...
// get brand info
$arResult["BRAND"] = B2BSKitShop::GetBrandInfo( $arResult["PROPERTIES"][$arParams["MANUFACTURER_ELEMENT_PROPS"]]["VALUE"], $ResizeMode );

// block characteristics
$arResult["DOP_PROPS"] = B2BSKitShop::GetCharacteristics( $arResult['IBLOCK_ID'], $arResult['IBLOCK_SECTION_ID'], $arResult['DISPLAY_PROPERTIES'], $arDopProps );
// block main props under offers props
$arResult["MAIN_PROPS"] = B2BSKitShop::GetCharacteristics( $arResult['IBLOCK_ID'], $arResult['IBLOCK_SECTION_ID'], $arResult['DISPLAY_PROPERTIES'], $arMainProps );

// Generate microrazmetka
$arResult['MICRORAZMETKA']['COMPONENT_TEMPLATE'] = ".default";
$arResult['MICRORAZMETKA']['NAME'] = B2BSKit::ExpandStringWithMacros( \Bitrix\Main\Config\Option::get( "kit.b2bshop", "MICRO_TOVAR_NAME", "" ), $arResult );
$arResult['MICRORAZMETKA']['DESCRIPTION'] = B2BSKit::ExpandStringWithMacros( \Bitrix\Main\Config\Option::get( "kit.b2bshop", "MICRO_TOVAR_DESCRIPTION", "" ), $arResult );
$arResult['MICRORAZMETKA']['ITEMCONDITION'] = \Bitrix\Main\Config\Option::get( "kit.b2bshop", "MICRO_TOVAR_ITEM_CONDITION", "" );
$arResult['MICRORAZMETKA']['PARAM_RATING_SHOW'] = "N";
$arResult['MICRORAZMETKA']['PAYMENTMETHOD'] = unserialize( \Bitrix\Main\Config\Option::get( "kit.b2bshop", "MICRO_TOVAR_PAYMENT_METHOD", "" ) );
$arResult['MICRORAZMETKA']['PRICECURRENCY'] = B2BSKit::ExpandStringWithMacros( \Bitrix\Main\Config\Option::get( "kit.b2bshop", "MICRO_TOVAR_PRICE_CURRENCY", "" ), $arResult );
$arResult['MICRORAZMETKA']['SHOW'] = "Y";
$arResult['MICRORAZMETKA']['AGGREGATEOFFER'] = "Y";
$arResult['MICRORAZMETKA']['ITEMAVAILABILITY'] = \Bitrix\Main\Config\Option::get( "kit.b2bshop", "MICRO_TOVAR_ITEM_AVAILABILITY_TRUE", "" );
...
// video
$arResult["VIDEO"] = B2BSKitShop::GetVideo( $arResult['PROPERTIES']['VIDEO']['VALUE'] );
...
if( $arParams['COLOR_IN_PRODUCT'] == 'Y' && $arResult["PROPERTIES"][$arParams["COLOR_IN_PRODUCT_LINK"]]["VALUE"])
{
	...
	{$ColorLink2 = B2BSKitShop::ColorLink2( $arModelsLinks, $arParams["COLOR_IN_PRODUCT_LINK"], $arParams['COLOR_IN_PRODUCT_CODE'], $colorCode, $arParams["ELEMENT_WIDTH_SMALL"], $arParams["ELEMENT_HEIGHT_SMALL"], 0, 0, $ResizeMode, $arParams['IBLOCK_ID'] );
...
$SortPropertyTree = B2BSKitShop::SortPropertyTree( $arResult["OFFER_TREE_PROPS"], $colorCode, $arResult['FIRST_OFFER'], $arResult["OFFERS_ID"], $arResult["CAN_BUY_OFFERS_ID"] );
...
$arResult['LI'] = B2BSKitShop::GenerateLiClasses( $arResult["OFFER_TREE_PROPS"], $arResult['OFFERS_ID'], $arResult['CAN_BUY_OFFERS_ID'], $arResult['FIRST_OFFER'] );

// Modification
$arResult['MODIFICATION'] = B2BSKitShop::GetModifications( $arResult['CAN_BUY_OFFERS_ID'], $colorCode, $arResult['MIN_PRICE']['CURRENCY'], $arParams["MODIFICATION_COUNT"] );
...
$arResult['MODAL_DATA'] = array(
		'basket_url' => Option::get('kit.b2bshop','URL_CART','/personal/cart/'),
		'add_wish' => 'Y',
);
