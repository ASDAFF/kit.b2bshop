...
$articleElementId = \Bitrix\Main\Config\Option::get( "kit.b2bshop", "OPT_ARTICUL_PROP", "" );
$articleOfferId = \Bitrix\Main\Config\Option::get( "kit.b2bshop", "OPT_ARTICUL_PROP_OFFER", "" );
$articleElementCode = '';
$articleOfferCode = '';
...
for($i = 0; $i < $cntItems; ++$i)
{
	...
	if( $arResult["ITEMS"][$i]['OFFERS'] )
	{
		for($j = 0; $j < sizeof( $arResult["ITEMS"][$i]['OFFERS'] ); ++$j)
		{
			...
			if( !isset( $HLParams[$colorCode] ) )
			{
				$PropArray = B2BSKitShop::GetValuesFromHL( $arResult["ITEMS"][$i]['OFFERS'][$j]['PROPERTIES'][$colorCode]['USER_TYPE_SETTINGS']['TABLE_NAME'], $arResult['CATALOG']['IBLOCK_ID'], $colorCode );
				$HLParams[key( $PropArray )] = $PropArray[key( $PropArray )];
			}
			...
			// $arResult["ITEMS"][$i]['MIN_PRICE'] = B2BSKitShop::GetMinPrice ( $arResult["ITEMS"][$i]['MIN_PRICE'], $arResult["ITEMS"][$i]['OFFERS'][$j]["MIN_PRICE"] );
			...
			if( $arParams['OFFER_TREE_PROPS'] )
			{
				for($k = 0; $k < sizeof( $arParams['OFFER_TREE_PROPS'] ); ++$k)
				{
					if( $arResult["ITEMS"][$i]['OFFERS'][$j]['PROPERTIES'][$arParams['OFFER_TREE_PROPS'][$k]]['VALUE'] && $arResult["ITEMS"][$i]['OFFERS'][$j]['PROPERTIES'][$arParams['OFFER_TREE_PROPS'][$k]]["USER_TYPE"] == 'directory' )
					{
						...
						{
							$PropArray = B2BSKitShop::GetValuesFromHL( $arResult["ITEMS"][$i]['OFFERS'][$j]['PROPERTIES'][$arParams['OFFER_TREE_PROPS'][$k]]['USER_TYPE_SETTINGS']['TABLE_NAME'], $arResult['CATALOG']['IBLOCK_ID'], $arParams['OFFER_TREE_PROPS'][$k] );
							$HLParams[key( $PropArray )] = $PropArray[key( $PropArray )];
							unset( $PropArray );
						}
	...
	else
	{
		...
		{
			/*
			 * $arResult["MORE_PHOTO_JS"][$IdItem][0] = B2BSKitShop::GetImagesIfOffers (
			 * $arResult["ITEMS"][$i]["PREVIEW_PICTURE"],
			 * $arResult["ITEMS"][$i]["DETAIL_PICTURE"],
			 * $arResult["ITEMS"][$i]['PROPERTIES']['ADD_PICTURES']['VALUE'],
	 ...
	 * {
	 * $arResult["MORE_PHOTO_JS"][$IdItem] = B2BSKitShop::reOneColor ( $arResult["MORE_PHOTO_JS"][$IdItem], $arParams["LIST_WIDTH_SMALL"], $arParams["LIST_HEIGHT_SMALL"], $arParams["LIST_WIDTH_MEDIUM"], $ResizeMode );
	 * }
	 */
	...
	$SortPropertyTree = B2BSKitShop::SortPropertyTree( $arResult["ITEMS"][$i]["OFFER_TREE_PROPS"], $colorCode, $arResult["ITEMS"][$i]['FIRST_OFFER'], $arResult["ITEMS"][$i]['OFFERS_ID'], $arResult["ITEMS"][$i]['CAN_BUY_OFFERS_ID'] );

	$arResult["ITEMS"][$i]['FIRST_OFFER'] = $SortPropertyTree['FIRST_OFFER'];
	$arResult["ITEMS"][$i]["OFFER_TREE_PROPS"] = $SortPropertyTree['PROPERTIES'];

	$arResult["ITEMS"][$i]['LI'] = B2BSKitShop::GenerateLiClasses( $arResult["ITEMS"][$i]["OFFER_TREE_PROPS"], $arResult["ITEMS"][$i]['OFFERS_ID'], $arResult["ITEMS"][$i]['CAN_BUY_OFFERS_ID'], $arResult["ITEMS"][$i]['FIRST_OFFER'] );
...
if( $arParams['COLOR_IN_PRODUCT'] == 'Y' && $arParams["COLOR_IN_SECTION_LINK"] == 2 && $arParams["COLOR_IN_PRODUCT_LINK"] )
{
	...
	if( sizeof( $arModelsLinks ) > 0 )
	{
		...
		$ColorLink2 = B2BSKitShop::ColorLink2( $arModelsLinks, $arParams["COLOR_IN_PRODUCT_LINK"], $arParams['COLOR_IN_PRODUCT_CODE'], $colorCode, 0, 0, 0, 0, $ResizeMode, $arParams['IBLOCK_ID'] );
...
/*
$priceIds = unserialize( \Bitrix\Main\Config\Option::get( "kit.b2bshop", "PRICE_CODE_SECTION", "" ) );
...
if( $ids && $priceIds )
{
	...
	foreach( $ids as $id => $val )
	{
		...
		{
			$forKitPrice[$idPrice]['ITEMS'][$id]['ID'] = $id;
			$forKitPrice[$idPrice]['ITEMS'][$id]['IBLOCK_ID'] = $val['IBLOCK_ID'];
			$discounts = array();
			...
			$forKitPrice[$idPrice]['ITEMS'][$id]['PRICES'] = array();
			$forKitPrice[$idPrice]['ITEMS'][$id]['MIN_PRICE'] = array();
			$discountPrice = \CCatalogProduct::CountPriceWithDiscount( $price['PRICE'], $price['CURRENCY'], $discounts );

			$forKitPrice[$idPrice]['ITEMS'][$id]['PRICES'][$idPrice] = array(
					'ID' => $idPrice,
					'VALUE' => $discountPrice,
					'PRINT_VALUE' => CurrencyFormat( $discountPrice, $price['CURRENCY'] ),
					'CURRENCY' => $price['CURRENCY']
			);
			$forKitPrice[$idPrice]['ITEMS'][$id]['MIN_PRICE'] = $forKitPrice[$idPrice]['ITEMS'][$id]['PRICES'][$idPrice];
		}
	}

	if( $forKitPrice )
	{
		foreach( $forKitPrice as $idPrice => $items )
		{
			if( Bitrix\Main\Loader::includeModule( "kit.price" ) )
			{
				$items = KitPrice::ChangeMinPrice( $items );
			}
