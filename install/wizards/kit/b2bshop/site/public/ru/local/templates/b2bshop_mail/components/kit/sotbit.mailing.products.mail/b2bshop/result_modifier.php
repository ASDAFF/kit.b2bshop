	...
	foreach ($arResult['ITEMS'] as $key => $item)
	{
		...
		else
		{
			if($arParams['ORDER_ID'] > 0)
			{
				...
				if($userId > 0)
				{
					$offersList = CCatalogSKU::getOffersList($item['ID'], 0,
							array('ACTIVE' => 'Y'), array(), array());
					if($offersList[$item['ID']])
					{
						$minPrice = array();
						foreach($offersList[$item['ID']] as $offer)
						{
							$arPrice = CCatalogProduct::GetOptimalPrice($offer['ID'], 1, CUser::GetUserGroup($userId));
							
							if(!$minPrice || $arPrice['RESULT_PRICE']['BASE_PRICE'] < $minPrice['PRICE'])
							{
								$minPrice['PRICE'] = $arPrice['RESULT_PRICE']['BASE_PRICE'];
								$minPrice['DISCOUNT_PRICE'] = $arPrice['RESULT_PRICE']['DISCOUNT_PRICE'];
							}
						}
						if($minPrice['PRICE'] > 0)
						{
							$item['MIN_PRICE']['VALUE'] = $minPrice['PRICE'];
							$item['MIN_PRICE']['DISCOUNT_VALUE'] = $minPrice['DISCOUNT_PRICE'];
							$item['MIN_PRICE']['PRINT_VALUE'] = CurrencyFormat($item['MIN_PRICE']['VALUE'],$arPrice['PRICE']['CURRENCY']);
							$item['MIN_PRICE']['PRINT_DISCOUNT_VALUE'] = CurrencyFormat($item['MIN_PRICE']['DISCOUNT_VALUE'],$arPrice['PRICE']['CURRENCY']);
						}
					}
				}
			}
