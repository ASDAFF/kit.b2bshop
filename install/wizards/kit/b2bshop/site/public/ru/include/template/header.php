				...
				<div class="col-sm-16">
					<div class='row'>
						<div class="col-sm-12 col-sm-push-12 xs-padding-no">
							<div class="logo">
								<a href="<?=SITE_DIR?>"><img class="logo-miss" src="<?=Option::get("kit.b2bshop", "LOGO", "")?>" width="534" height="208" title="<?=$Template->getSiteName()?>" alt="<?=$Template->getSiteName()?>"/></a>
							</div>
						</div>
						<div class="col-sm-12 col-sm-pull-12 sm-padding-left-no">
							<?if(
									Loader::includeModule('kit.regions')
									&& COption::GetOptionString("kit.b2bshop","USE_MULTIREGIONS","Y") == 'Y')
							{?>
							...
							<section class="header-top-block-left">
								...
								<div class="wrap-phone">
									...
									if(
										Loader::includeModule('kit.regions') &&
										COption::GetOptionString("kit.b2bshop","USE_MULTIREGIONS","Y") == 'Y' &&
										$_SESSION["KIT_REGIONS"]["UF_PHONE"])
									{
										if(is_array($_SESSION["KIT_REGIONS"]["UF_PHONE"]))
										{
											foreach($_SESSION["KIT_REGIONS"]["UF_PHONE"] as $phone)
											{
												echo '<p>'.$phone.'</p>';
											}
										}
										...
										{
											echo '<p>'.$_SESSION["KIT_REGIONS"]["UF_PHONE"].'</p>';
										}
									}
									else
									{
